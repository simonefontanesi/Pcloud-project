...
dbName = 'a1234'
collUsers = 'utenti'
stockDB = firestore.Client.from_service_account_json('credentials.json', database=dbName) #fare qualcosa

####NUOVA PARTE
dbStock_list='azioni'
collAzioni = 'stocks'

DBazioni=firestore.Client.from_service_account_json('credentials.json', database=dbStock_list)

usersDB = {}

DBstock_list={}

storage_client = storage.Client.from_service_account_json('credentials.json')  # accedo al cloud storage
...
def save_model(model):
    file_name = model
    #temp_model_path = f'/tmp/{model}' #ONLINE

    bucket_name = 'bucket_ml_models'
    bucket = storage_client.bucket(bucket_name)
    blob = bucket.blob(file_name)
    blob.upload_from_filename(f'/tmp/{model}')
    #os.remove(file_name)
"""
def get_model(model):
    bucket_name='bucket_ml_models'
...
def get_model(model_filename):
    # Imposta il nome del bucket
    bucket_name = 'bucket_ml_models'  # Modifica con il nome del tuo bucket

    # Imposta il client di Cloud Storage
    #client = storage.Client()

    # Specifica il bucket
    bucket=storage_client.bucket(bucket_name)

    # Specifica il blob (il file nel bucket)
    blob = bucket.blob(model_filename)

    # Scarica il file temporaneamente in memoria o una cartella temporanea
    #temp_model_path = f'tmp/{model_filename}' #LOCALE
    temp_model_path = f'/tmp/{model_filename}' #ONLINE
    # Scarica il file nel percorso temporaneo
    blob.download_to_filename(temp_model_path)

    # Carica il modello con joblib
...
def add_stock():
    ...
    try:
        ...
        lr = LinearRegression()
        lr.fit(X, y)
        #temp_model_path = f'/tmp/MODELLO_univariato_{stock}.joblib'  # ONLINE
        model=f'MODELLO_univariato_{stock}.joblib'  # ONLINE

        joblib.dump(lr,f'/tmp/MODELLO_univariato_{stock}.joblib') #ONLINE
        #joblib.dump(lr,f'MODELLO_univariato_{stock}.joblib') #LOCALE

        save_model(model) #ONLINE

        #save_model(f'MODELLO_univariato_{stock}.joblib') #LOCALE
        ...
        # Rimuovi il file temporaneo se necessario
        os.remove(f'/tmp/MODELLO_univariato_{stock}.joblib')
        ...
        if stock not in user_stocks:
            ...
            #save_stock_data_batch(stock,last_date)
...
def calculate_LOG_returns(prevision,history):
    ...
    d="""
    I log returns sono la differenza logaritmica tra il prezzo di un asset in due diversi momenti temporali.
    Sono un modo per misurare la variazione percentuale del valore di un asset nel tempo.
    """
